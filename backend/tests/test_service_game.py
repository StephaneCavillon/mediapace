...
def test_game_creation(context):
  ...
  game = games_service.create(game_data, context['test_user'].id)
...
def test_game_delete(context):
  ...
  game = games_service.create(game_data, str(context['test_user'].id))
...
def test_user_game_list(context, seed_games):
  listed_games = games_service.list(str(context['test_user'].id))

  assert len(listed_games) == 3
  assert listed_games[0].title == 'Test Game 1'
  assert listed_games[0].platform == 'PC'
  assert listed_games[0].completion_time == 10.5
  assert listed_games[0].user.id == context['test_user'].id
  assert listed_games[1].title == 'Test Game 2'
  assert listed_games[1].platform == 'PS5'
  assert listed_games[1].completion_time == 20.0
  assert listed_games[1].user.id == context['test_user'].id
  assert listed_games[2].title == 'Test Game 3'
  assert listed_games[2].platform == 'Xbox'
  assert listed_games[2].completion_time == 30.0
  assert listed_games[2].user.id == context['test_user'].id
