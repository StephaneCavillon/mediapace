...
class BooksService:
  ...
  def list(self, user: str):
    return Book.get(Book.user == user)
  ...
  def update(self, book_id: int, data: BookUpdate):
    row_updated = Book.update(**data.model_dump()).where(Book.id == book_id).execute()
    if row_updated == 0:
      raise HTTPException(status_code=404, detail='Book not found')
    return Book.get_by_id(book_id)

  def delete(self, book_id: int):
    row_deleted = Book.delete().where(Book.id == book_id).execute()
    if row_deleted == 0:
      raise HTTPException(status_code=404, detail='Book not found')
    return Book.get_by_id(book_id)
